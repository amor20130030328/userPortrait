...
import com.gy.userportrait.utils.CarrierUtils;
...
public class YearBase {
    ...
    public static void main(String[] args) throws Exception{
        ...
        String path = "D:\\source\\project\\userportrait\\producerData\\output\\user.log";
        ...
        while(true){
            ...
            user.setRegisterTime(generateRegisterTime());
            user.setUsetype(random.nextInt(3)+"");
            user.setTelphone(generateTelephone());
            user.setSex(count % 2 == 0 ? "0":"1");
            user.setUsername(getName());
            user.setUserid(UUID.randomUUID().toString());
            System.out.println(user);
            list.add(user);
            writer.write(user.format());
            writer.newLine();
        }
        writer.close();
